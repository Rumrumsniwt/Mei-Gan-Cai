...
import itchat, requests, time, json, math
import re
...
def text_reply(msg):
	if msg['FromUserName']==myUserName:
		pattern=r'[ | | ]'
		resultafter=re.split(pattern,msg['Text'])
		page=int(resultafter[0])
		ID=int(resultafter[1])
		item=int(resultafter[2])
		module=resultafter[3]
		...
		itchat.send_image("C:/Users/lenovo/Desktop/SU2019-450/python test/result.png",'filehelper')
#		itchat.send_msg(u"[%s]Received from friend @%s: %s\n" %
#			(time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(msg['CreateTime'])),
#			msg['User']['NickName'],
#			msg['Text']),'filehelper')
#		return u'I am absent right now. \n Already received your message: %s\n' %(msg['Text'])
